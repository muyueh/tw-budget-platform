      ...
      <div className={cx({"cp-loading":this._waiting})}>
        <ul className="nav nav-tabs">
          <li role="presentation" className={cx({active:this.state._subnav == 'all'})}>
            <a onClick={this.doNav.bind(this,'all')} href={Util.site_url("table/"+this.props.budget_id+"/all")}>總科目表</a>
          </li>
          <li role="presentation" className={cx({active:this.state._subnav == 'topname'})} >
            <a onClick={this.doNav.bind(this,'topname')} href={Util.site_url("table/"+this.props.budget_id+"/topname")}>只看款</a>
          </li>
          <li role="presentation" className={cx({active:this.state._subnav == 'depname'})} >
            <a onClick={this.doNav.bind(this,'depname')} href={Util.site_url("table/"+this.props.budget_id+"/depname")}>只看項</a>
          </li>
          <li role="presentation" className={cx({active:this.state._subnav == 'category'})} >
            <a onClick={this.doNav.bind(this,'category')} href={Util.site_url("table/"+this.props.budget_id+"/category")}>只看目</a>
          </li>
        </ul>
